...
int initMax(int idx, int st, int en) {
	if (st == en) return maxTree[idx] = arr[st]; // leaf
	int mid = (st + en) >> 1;
	return maxTree[idx] = max(initMax(idx << 1, st, mid), initMax((idx << 1) + 1, mid + 1, en));
}
...
int findMax(int st, int en, int idx, int left, int right) {
	if (left > en || right < st) return 0;
	if (left <= st && en <= right) return maxTree[idx];
	int mid = (st + en) >> 1;
	return max(findMax(st, mid, idx << 1, left, right), findMax(mid + 1, en, (idx << 1) + 1, left, right));
}
